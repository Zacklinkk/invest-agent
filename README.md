...
投资分析编排插件 — 通过三级调度（L1 直答 / L2 单专家 / L3 多 Agent 团队）协调 20+ 个金融分析 skill，完成选股、择时、交易方案三大投资决策。
...
| L2 | 单维度 + 完整工作流 | spawn 1 个 subagent | "分析茅台财报"、"DCF 估值比亚迪" |
...
用户请求
  ...
  │
  ├─ L2: spawn general-purpose agent + bypassPermissions
  │     └─ 单分析师执行完整流程
  │
  └─ L3: TeamCreate 三波执行
        ├─ Wave 1: 6 个分析师并行
...
### 交易方案（How to execute?）
- 6 位分析师各出 trading_suggestion → 交易策划师综合判定
...
```
/invest quick 茅台PE          # L1: 快速查询
/invest deep 茅台财报          # L2: 单维度深度分析
/invest full 贵州茅台          # L3: 全维度分析 + 报告
/invest compare 茅台 五粮液    # L3: 多标的对比
```
...
**核心分析层**: 财报分析, damodaran-valuation, industry-research, technical-analysis, al-brooks-price-action, taleb-risk-philosophy
...
2. **Plugin Agent 不可直接调用**: Agent tool 的 `subagent_type` 不支持 plugin agent，L2/L3 均使用 `general-purpose` + `bypassPermissions`
3. **所有输出使用中文**: 8 个 agent 系统提示均包含中文输出要求
...
├── .claude-plugin/
│   └── plugin.json              # 插件清单
├── agents/                      # 8 个 agent 定义（供 Claude 自动匹配）
